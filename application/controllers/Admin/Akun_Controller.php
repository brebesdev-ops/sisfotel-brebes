...
class Akun_Controller extends CI_Controller
{
	...
	{
		if ($this->session->userdata('role') === '2') {
			$data['akun'] = $this->db->query("SELECT * FROM tb_user WHERE is_approved = '0'")->result();
			$x['admin'] = $this->db->get_where('auth', ['id' => $this->session->userdata('id')])->row_array();
			$this->load->view('layout/backend/header');
			$this->load->view('layout/backend/topbar', $x);
			$this->load->view('layout/backend/sidebar');
	...
	{	
		if ($this->session->userdata('role') === '2') {
			$where = array('id' => $id);
			$data['berita'] = $this->db->query("SELECT * FROM tb_user WHERE id='$id'")->result();
			$x['admin'] = $this->db->get_where('auth', ['id' => $this->session->userdata('id')])->row_array();
