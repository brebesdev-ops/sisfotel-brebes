...
class Permohonan_Controller extends CI_Controller
{
	...
	{	
		...
		$is_approve_kasi               = $this->input->post('nama');
	...
	public function update_kasi()
	{
		...
		$id					= $this->input->post('id');
		$is_approve_kasi      = $this->input->post('is_approve_kasi');

	   
		$data = array(
			'is_approve_kasi'				=> $is_approve_kasi
		);
		...
		$this->M_Permohonan->update_data('tb_permohonan',$data, $where);
		redirect('kasi/permohonan/detail/'.$id);     
	}
	public function update_kasi_tolak()
	{
